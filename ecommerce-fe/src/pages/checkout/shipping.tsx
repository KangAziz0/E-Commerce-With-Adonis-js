...
export default function ShippingPage({ onCourierSelected }: Props) {
  ...
  const { data: dataRates } = useSelector((state: RootState) => state.checkout.rates);
  ...
  const sortedRates = useMemo(() => [...rates].sort((a, b) => a.price - b.price), [rates]);
        ...
        <Row className="g-4">
          <Col lg={8}>
            <Card className="shipping-card mb-4">
              <Card.Body className="p-4">
                ...
                <RecipientAddressForm onChange={(address: SelectedAddress | null) => setDestinationAreaId(address?.area_id ?? "")} />
                <div className="mt-3">
                  <Button onClick={handleGetRates} disabled={loading || !destinationAreaId}>
                    {loading ? <><Spinner size="sm" className="me-2" />Mencari...</> : "Cek Pengiriman"}
                  </Button>
                </div>
              </Card.Body>
            </Card>
            ...
            <Card className="shipping-card">
              <Card.Body className="p-4">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  {selectedRate && <Badge bg="success">{selectedRate.courier_name} dipilih</Badge>}
                </div>

                {searched && sortedRates.length > 0 ? (
                  <Row className="g-3">
                    {sortedRates.map((rate) => (
                      <Col md={6} key={`${rate.courier_code}-${rate.courier_service_code}`}>
                        <CourierCard
                          rate={rate}
                          selected={
                  ...
                  <p className="text-muted mb-0">Pilih alamat terlebih dahulu untuk melihat opsi pengiriman.</p>
                )}
              </Card.Body>
            </Card>
          </Col>
          ...
          <Col lg={4}>
            <Card className="shipping-card sticky-top" style={{ top: 20 }}>
              <Card.Body className="p-4">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h4 className="mb-0">Metode Pembayaran</h4>
                  <a href="#" className="text-success fw-semibold text-decoration-none">Lihat Semua</a>
                </div>
                {PAYMENT_METHODS.map((method, index) => (
                  <div className="payment-item d-flex justify-content-between align-items-center" key={method}>
                    <span>{method}</span>
                    <span className={`payment-dot ${index === 0 ? "active" : ""}`} />
                  </div>
                ))}
              </Card.Body>

              <div className="total-box p-4">
                <h4 className="mb-3">Cek ringkasan transaksimu, yuk</h4>
                ...
                <Button className="pay-button w-100 py-3">Bayar Sekarang</Button>
              </div>
            </Card>
          </Col>
        </Row>
